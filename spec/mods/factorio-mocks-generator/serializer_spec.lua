insulate("[#UNIT] serializer", function()
  ...
  it("should call serpent.block()", function()
    ...
    serializer.serialize(test_table)
  ...
  describe("output", function()
    it("should start with 'return'", function()
      ...
      local result = serializer.serialize(input)

      assert.truthy(result:match("^return "))
    end)
  end)

  describe("_flat_deep_copy", function()
    it("should handle non-table values", function()
      assert.are.equal(42, serializer._flat_deep_copy(42))
      assert.are.equal("test", serializer._flat_deep_copy("test"))
      assert.are.equal(true, serializer._flat_deep_copy(true))
      assert.are.equal(nil, serializer._flat_deep_copy(nil))
    end)

    it("should create a copy of simple tables", function()
      local original = {a = 1, b = 2}
      local copy = serializer._flat_deep_copy(original)
...
insulate("[#INTEGRATION] serializer", function()
  ...
  it("should handle non-table values", function()
    local result = serializer.serialize({test = "simple"})
